...
constexpr uint64_t kPurgeBufferMs = 3600 * 1000;
...
static void SelectObsoleteFiles(
  ...
  for (const auto &candidate : all_files) {
    ...
    if (candidate_modification_time < manifest_modification_time) {
      // Temp hack: only delete files that are at least one hour older
      // than the manifest modification time, to avoid deleting new files that
      // are generated by compaction but not included in the manifest yet. This
      // can be removed once we have a more robust mechanism implemented.
      if (candidate_modification_time <
          manifest_modification_time - kPurgeBufferMs) {
        ...
        Log(InfoLogLevel::DEBUG_LEVEL, cfs.info_log_,
            ...
            "time %lu is too close to the current epoch manifest file's "
