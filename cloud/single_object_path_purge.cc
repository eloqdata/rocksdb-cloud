...
static void SelectObsoleteFiles(
  ...
  for (const auto &candidate : all_files) {
    ...
    if (candidate_modification_time < manifest_modification_time) {
      ...
      // than the manifest modification time, to avoid delete new files which
      // generated by compaction but not included in the manifest yet.
      // This can be removed once we have a more robust mechanism
      // implemented.
      if (candidate_modification_time <
          manifest_modification_time - 3600 * 1000) {
